...
Shader "Custom/VertexColor4" {
    ...
    Pass {
        ...
        VertexOutput vert(VertexInput v) {
            ...
            if(posDist < 3.0) {
            	o.pos = UnityObjectToClipPos(float4(v.v.x,v.v.y,v.v.z - min(max((v.uv.z-1.0+1.1*_sSlider),0.0)*20,1.0) * 0.5,v.v.w).xyzw);
            	o.col.rgb = float3(0.75+((v.uv.z-0.75)),0.75-((v.uv.z-0.25)),0.75+((v.uv.z-0.75))).rgb;
            }
            //else if (cposDist < 3.0) {
            //	o.pos = mul(UNITY_MATRIX_MVP, float4(v.v.x,v.v.y,v.v.z - min(max((v.uv.z-1.0+1.1*_sSlider),0.0)*20,1.0) * 0.5,v.v.w).xyzw);
            //	o.col.rgb = float3(0.75+((v.uv.z-0.75)),0.75-((v.uv.z-0.25)),0.75+((v.uv.z-0.75))).rgb;
            //}
            else {
